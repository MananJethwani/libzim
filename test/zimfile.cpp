...
void checkEquivalence(const zim::File& zimfile1, const zim::File& zimfile2)
{
  ...
  ASSERT_NE(0, zimfile1.getCountArticles()); // ==> below loop is not a noop
...
TEST(ZimFile, openByFD)
{
  const zim::File zimfile1("./data/wikibooks_be_all_nopic_2017-02.zim");
  // XXX: will this work on windows?
  const int fd = open("./data/wikibooks_be_all_nopic_2017-02.zim", O_RDONLY);
  const zim::File zimfile2(fd);
